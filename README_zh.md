...
支持 `Paper`、`Purpur` 及 `Folia`。Folia 使用区域线程模型，插件已针对实体调度器、区域调度器和全局调度器进行适配。

要求使用 JAVA 21 及以上

+ 支持上游已有的 `1.20.x`、`1.21.x` 版本
+ 支持 `1.21.11`
+ 支持 `26.1.2` 及后续 `26.x` 版本
...
- [CommandAPI](https://commandapi.jorel.dev)（11.0.0）

许可证：Apache-2.0（与上游保持一致）。
